...
import { CalendarIcon, Upload, PlusCircle, AlertCircle, FileText, Users, DollarSign, Check } from 'lucide-react';
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow-sm border border-gray-200">
      <div className="mb-6 text-center">
        <h2 className="text-2xl font-bold text-gray-800">Create New Job Request</h2>
        <p className="text-gray-500 mt-1">Fill out the form below to post a new cosmetic surgery job request</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='space-y-6'>
        ...
        <div className="space-y-2">
            ...
            {AttachmentUrls?.length > 0 && !uploadingFiles && (
              <div className="mt-3">
                <p className='text-sm text-green-600 flex items-center'>
                  ...
                  {`${AttachmentUrls.length} ${AttachmentUrls.length === 1 ? 'file' : 'files'} uploaded successfully`}
                </p>
                
                {/* Preview of uploaded files */}
                <div className="grid grid-cols-3 md:grid-cols-4 gap-2 mt-2">
                  {AttachmentUrls.slice(0, 4).map((url, index) => (
                    <div key={index} className="relative h-16 bg-gray-100 rounded overflow-hidden">
          ...
          <p className="text-xs text-gray-500">Upload images or videos related to your request (optional)</p>
        </div>
        ...
        <div className="flex justify-center pt-4">
          <Button
            type='submit'
            className={`px-8 py-3 rounded-md flex items-center justify-center text-white font-medium ${loading ? 'bg-indigo-400' : 'bg-indigo-600 hover:bg-indigo-700'} transition-colors`}
