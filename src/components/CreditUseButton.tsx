...
interface CreditUseButtonProps {
  ...
  variant?: 'offer'; // Optional variant, only changes behavior if it's "offer"
}

export default function CreditUseButton({
  jobId,
  surgeonId,
  chatPartnerName,
  variant,
}: CreditUseButtonProps) {
  ...
  return (
    ...
    >
      ...
      {loading
        ? 'Processing...'
        : variant === 'offer'
        ? 'Use Credit to unlock all offers for this job'
        : 'Use Credit to unlock all replies of this job post'}
    </button>
  );
}
