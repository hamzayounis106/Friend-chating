...
import { Package, Shield, Crown, CheckCircle, CreditCard, Loader } from 'lucide-react';
...
const packages = [
  ...
  },
  {
    ...
    features: ['VIP access', 'Three consultations', 'Priority scheduling', '15% discount']
  }
];
...
export default function CreditPackages() {
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">Credit Packages</h1>
        ...
        {packages.map((pkg) => (
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div>
                <h2 className={`text-2xl font-bold ${pkg.textColor}`}>{pkg.title}</h2>
                <p className="text-gray-600">{pkg.credits} Credit{pkg.credits > 1 ? 's' : ''}</p>
              </div>
              <div>{pkg.icon}</div>
            </div>
      ...
      <div className="max-w-md mx-auto bg-white p-6 rounded-lg shadow-md">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">Complete Your Purchase</h2>
        
        {selectedPackage && (
          <div className="mb-6 p-3 bg-gray-50 rounded-md">
            <p className="font-medium text-gray-700">
              Selected Package: <span className="text-blue-600">{packages.find(p => p.id === selectedPackage)?.title}</span>
            </p>
            <p className="text-gray-600">
        ...
        <div className="mb-6">
          <h3 className="text-sm font-medium text-gray-700 mb-2">How Credits Work:</h3>
          <ul className="text-sm text-gray-600 space-y-1 pl-5 list-disc">
            <li>Credits can be used for any consultation with our surgeons</li>
            <li>Credits don't expire and remain in your account until used</li>
            <li>You can assign credits to specific jobs when scheduling consultations</li>
          </ul>
        </div>
